...
try:
    ...
    exit(1)
if not study in ["facetalk", "wls", "awl", "ewl"]:
...
def insert_col_header(application_path):
    ...
    for file in files:
        ...
        if study in ["facetalk", "wls"]:
            ...
            col_list = ['Left Top Longest', 'Right Top Longest', 'Left Bottom Longest', 'Right Bottom Longest', 'Left Top',
                    'Right Top', 'Left Bottom', 'Right Bottom', 'Center', 'Total Look', 'Trial Length', 'Attention']
        col_count = 5
        list_count = 0
        for sheet in wb:
            if sheet.title == 'AVERAGES ACROSS CODERS':
            ...
            for sheet in wb.worksheets: #for each sheet in excel file
                ...
                for row in sheet.iter_rows():
                    ...
                    if code == 'B': #if beginning of trial
                        ...
                        if row[1].value - start_time < 30: #if before 30 frames
                            if (row[1].value - start_time) + row[3].value > 30: #check trial is before 30 frames and goes over 30 frames
                                difference = (row[1].value - start_time) - 30 #calculate the time that is before 30 frames
                                look = row[3].value + difference #add the difference to get the time that is over 30 frames
                        ...
                        col_count = 5
                        list_count = 0
                        data = [lt_max, rt_max, lb_max, rb_max, lt_sum, rt_sum, lb_sum, rb_sum, c_sum, total_look, trial_length, attention]
                        while col_count <= 16: #insert data to excel file
                            sheet.cell(row=row_count, column=col_count).value = data[list_count]
                            col_count += 1
                            list_count += 1
                        row_count += 1 # go to next row 
...
def insert_dis_col_header(application_path):
    ...
    for file in files:
        ...
        if study in ["facetalk", "wls"]:
            ...
            col_list = ['Left Top Longest', 'Left Top Longest Dis','Right Top Longest', 'Rt Top Longest Dis', 'Left Bot Longest',
                    'Left Bot Longest Dis', 'Right Bottom Longest', 'RT Bot Longest Dis', 'Left Top', 'LT Dis',
                    'Right Top', 'RT Dis', 'Left Bottom', 'LB Dis', 'Right Bottom', 'RB Dis', 'Center', 'C Dis', 'Total Look', 'Total Dis', 'Trial Length',
                    'Trial Dis', 'Attention']
        col_count = 1
        list_count = 0
        ...
        for file in files:
            ...
            for row1, row2 in zip(sheet1, sheet2):
                ...
                if cur_cell.value > 15 or cur_cell.value < -15:#color read if dis is greater than 15 or less than -15
                    ...
                    cur_cell.fill = yellowFill
                #calculate center and center disp
                c_sum = (row1[12].value + row2[12].value) / 2
                sheet3.cell(row=new_row_count, column = 17).value = c_sum
                c_sum_dis = row1[12].value - row2[12].value
                sheet3.cell(row=new_row_count, column = 18).value = c_sum_dis
                ...
                if cur_cell.value > 15 or cur_cell.value < -15:#color read if dis is greater than 15 or less than -15
                    ...
                    cur_cell.fill = yellowFill
                #calculate total look and total look disp
                total_look_sum = (row1[13].value + row2[13].value) / 2
                sheet3.cell(row=new_row_count, column = 19).value = total_look_sum
                total_look_dis = row1[13].value - row2[13].value
                sheet3.cell(row=new_row_count, column = 20).value = total_look_dis
                ...
                if cur_cell.value > 15 or cur_cell.value < -15:#color read if dis is greater than 15 or less than -15
                    ...
                    cur_cell.fill = yellowFill
                #calculate trial length and trial length disp
                trial_length_sum = (row1[14].value + row2[14].value) / 2
                sheet3.cell(row=new_row_count, column = 21).value = trial_length_sum
                trial_length_dis = row1[14].value - row2[14].value
                sheet3.cell(row=new_row_count, column = 22).value = trial_length_dis
                cur_cell = sheet3.cell(new_row_count, 22)
                if cur_cell.value > 15 or cur_cell.value < -15:#color read if dis is greater than 15 or less than -15
                    cur_cell.fill = redFill
                else:   
                    cur_cell.fill = yellowFill
                
                attention_sum = (row1[15].value + row2[15].value) / 2
                sheet3.cell(row=new_row_count, column = 23).value = attention_sum
